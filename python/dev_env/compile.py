...
for lw in line_width:
    for md in model_depth:
        for dw in data_width:
            ...
            with open("./apps/tcc_beacon/src/models/CMakeLists.txt", "w+") as file:
                file.write(f'''target_sources(tcc_beacon PUBLIC ${{CMAKE_CURRENT_LIST_DIR}}/lw{lw}_md{md}.cc)''')

            with open("./apps/tcc_beacon/main.cpp", "r") as file:
                data = file.read()
                data = data.replace(last_model, f"lw{lw}_md{md}")             
            with open("./apps/tcc_beacon/main.cpp", "w") as file:
                file.write(data)
            ...
            subprocess.run(f'mv ./images/tcc_beacon.elf ./images/tcc_lw{lw}_md{md}_{dw}.elf'.split(' '),
                                  stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
